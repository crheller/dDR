...
cov_noise = 0 #np.random.normal(0, 1, (nUnits, nUnits))
...
np.fill_diagonal(covind, np.diag(sh.generate_full_rank_cov(evecs, cov_noise)))

# DATASET 2: 1-D noise, not aligned (one big dimension, all others small)
svs = np.append(1, 0.3 / np.arange(2, nUnits+1)**(1/2))
cov1 = sh.generate_full_rank_cov(evecs * svs, cov_noise)

# DATASET 3: 1-D noise, aligned, other dimensions small
svs = np.append(1, 0.3 / np.arange(2, nUnits+1)**(1/2))
cov1a = sh.generate_full_rank_cov(evecsA * svs, cov_noise)

# DATASET 4:  2-D noise
svs = np.append(1, np.append(0.8, 0.2 / np.arange(3, nUnits+1)**(1/2)))
cov2 = sh.generate_full_rank_cov(evecs * svs, cov_noise)

# DATASET 5: 1/f noise
svs = 1/np.arange(1, nUnits+1)**(1/2)
cov1f = sh.generate_full_rank_cov(evecs * svs, cov_noise)
...
for tit, dim, x in zip(['Ind. Noise', '1-D', '1-D aligned', '2-D', '1/f'], d_full.keys(), xpos):
    ...
    # plot scree plots
    #ax[0, x].bar(range(nevals), evals[dim][:nevals] / evals[dim].sum(), width=1, edgecolor='white', color='grey')
    ...
    # summarize PCA vs. dDR performance with a scatter plot
    ax[2, x].scatter(d_pca_plot.mean(axis=1)[:-1], d_ddr_plot.mean(axis=1)[:-1], c=np.sqrt(krange[:-1])*3, edgecolor='white')
    ax[2, x].plot([0, lim], [0, lim], '--', color='grey')
    ax[2, x].set_xlim((0, lim))
    ax[2, x].set_ylim((0, lim))
    ax[2, x].set_xlabel(r"$PCA$ ($d'^2$)")
    ax[2, x].set_ylabel(r"$dDR$ ($d'^2$)")
